...
// std::mt19937 Agent::gen{std::random_device{}()};   Random number generator
...
std::optional<Order> Agent::agent_buy(float mkt_price) {
  ...
  return std::make_optional(
      Order(price, id, 0, simulator.get_current_time(), 0));
}
...
std::optional<Order> Agent::agent_sell(float mkt_price) {
  ...
  return std::make_optional(
      Order(price, id, 1, simulator.get_current_time(), 0));
}

std::optional<Order> Agent::prediction_engine() {
  float current_price = simulator.get_current_price();
  /*
  This first iteration is a completely random prediction engine. Therefore, the
  state of the agent will not be used. Instead the agent will randomly decide to
  buy, sell or hold, given the constraints of its cash and shares. Reminder that
  currently only one share can be traded at a time. 0 cash and 0 shares will
  throw an error (TODO: implement logic to remove agent from sim) An agent which
  has an order placed in an orderbook will have its cash/shares deducted until
  said order is filled.
  */
...
void Agent::place_order(Order &order) {
  if (order.getTradeType() == 0) {
    ...
    cash -=
        order.getPrice(); // Deduct cash immediately when placing a buy order
  } else {
...
char Agent::get_state() const { return state; }